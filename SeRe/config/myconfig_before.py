...
model = dict(
    ...
    # decode_head=dict(
    #     type='PSP',  # 解码头(decode head)的类别。 可用选项请参考 mmseg/models/decode_head。 changed: from psp to the current
    ...
    #     num_classes=9,  # need to change 分割前景的种类数目。 通常情况下，cityscapes 为19，VOC为21，ADE20k 为150。
...
data_root = '/media/xjw/data/seg_cl/myseg_cl/data'  # 数据的根路径。
...
data = dict(
    ...
    train=dict(  # 训练数据集配置
        ...
        data_root='/media/xjw/data/seg_cl/myseg_cl/data',  # 数据集的根目录。
        # img_dir='ori_data/images',  # 数据集图像的文件夹。
        # ann_dir='ori_data/annotations',  # 数据集注释的文件夹。
        # img_dir='new_data/images',  # 数据集图像的文件夹。
        # ann_dir='new_data/annotations',  # 数据集注释的文件夹。
        img_dir='xiangtan_cjdata/images/training',  # 数据集图像的文件夹。
        ann_dir='xiangtan_cjdata/annotations/training',  # 数据集注释的文件夹。
        # split='new_data/_4.txt',
    ...
    val=dict(  # 验证数据集的配置
        ...
        data_root='/media/xjw/data/seg_cl/myseg_cl/data',
        img_dir='xiangtan_cjdata/images/validation',
        ann_dir='xiangtan_cjdata/annotations/validation',
...
runner = dict(
    ...
    max_iters=160000) # need to change 全部迭代轮数大小，对于 EpochBasedRunner 使用 `max_epochs` 。
